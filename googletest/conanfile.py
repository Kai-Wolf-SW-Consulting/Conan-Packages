...
class GoogletestConan(ConanFile):
    name = "googletest"
    version = "1.8.1"
    description = "Google C++ test framework"
    url = "https://github.com/Kai-Wolf-SW-Consulting/Conan-Packages/googletest"
    homepage = "https://github.com/google/googletest"
    author = "Kai Wolf - SW Consulting <mail@kai-wolf.me>"
    license = "BSD-3-Clause"
    topics = ("gtest", "testing", "unittest")
    exports = ["LICENSE.md"]
    ...
    def configure(self):
        if self.settings.os == "Windows":
            if self.settings.compiler == "Visual Studio" and Version(
                    self.settings.compiler.version.value) <= "12":
                raise ConanInvalidConfiguration(
                    "Google Test {} does not support Visual Studio <= 12".format(self.version))

    def source(self):
        sha256 = "9bf1fe5182a604b4135edc1a425ae356c9ad15e9b23f9f12a02e80184c3a249c"
        tools.get(
            "{0}/archive/release-{1}.tar.gz".format(self.homepage, self.version), sha256=sha256)
    ...
    def package_info(self):
        if self.options.build_gmock:
            gmock_libs = ['gmock', 'gtest'
                         ] if self.options.no_main else ['gmock_main', 'gmock', 'gtest']
